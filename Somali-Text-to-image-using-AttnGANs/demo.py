...
#def translate_to_english(somali_text):
 #   translator = Translator()
  #  english_text = translator.translate(somali_text, src='so', dest='en').text
   # return english_text
...
#def translate_to_english(somali_text):
 #   translator = Translator(from_lang="somali", to_lang="english")
  #  english_text = translator.translate(somali_text)
   # return english_text
...
def demo_gan():
    ...
    st.subheader("Enter the description of the bird in Somali")
    somali_input = st.text_area("Write the bird description in Somali below")
    english_input = translate_to_english(somali_input)


    # Generate images for the translated English input
    if english_input:
        start_t = time.time()
        gen_example(dataset.wordtoix, algo, text=english_input)
        end_t = time.time()
        st.write(f"**Your Somali input**: {somali_input}")
        st.write(f"**Translated English input**: {english_input}")

        # Display the generated bird image and attention map
        ...
        # Expander for First Stage Images
